enum UserRole { farmer, merchant, customer, admin, priceUpdater, guest }
...
class HarvestPreviewModel {
  ...
  int get daysLeft => expectedDate.difference(DateTime.now()).inDays;

  factory HarvestPreviewModel.fromMap(Map<String, dynamic> data, String id) {
    return HarvestPreviewModel(
      id: id,
      farmerId: data['farmerId'] ?? '',
      cropName: data['cropName'] ?? '',
      cropNameTa: data['cropNameTa'] ?? '',
      expectedDate: data['expectedDate'] != null 
          ? DateTime.parse(data['expectedDate']) 
          : DateTime.now(),
      expectedYield: (data['expectedYield'] ?? 0).toDouble(),
      unit: data['unit'] ?? 'kg',
