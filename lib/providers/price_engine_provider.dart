...
class PriceEngineProvider extends ChangeNotifier {
  ...
  List<AggregatedPrice> _all = [];
  List<AggregatedPrice> get all => _all;
  ...
  Future<void> loadAll() async {
    try {
      ...
      final result = await _svc.db.listDocuments(
        ...
        queries: [Query.limit(100), Query.orderDesc('lastUpdated')],
      );
      _all = result.documents.map(_parseAggregated).toList();
      notifyListeners();
    } catch (e) {
      debugPrint('❌ Failed to load all prices: $e');
    }
  }
